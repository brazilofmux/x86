...
# The guest's clock is its instruction count (plus time spent blocked on
# the host): a compiler's "N seconds" line and the input-exhausted deadline
# then come out the same on a fast Mac, under -V and under qemu-user.
export X86_VCLOCK=1
fail=0
...
for mode in -i -j -V "-m 86 -V" "-m 386 -V"; do
    check "hello.com $mode" tests/dos/hello.out $DM $mode tests/dos/hello.com
    check "exe1.exe $mode"  tests/dos/exe1.out  $DM $mode tests/dos/exe1.exe
    check "exec.com $mode"  tests/dos/exec.out  $DM $mode tests/dos/exec.com
...
for mode in -j -V; do
    rm -f tmp/text.png
    $DM -W $mode -G tmp/text.png tests/dos/text.com </dev/null >/dev/null 2>&1
...
for mode in -i -j -V; do
    check "dpmi $mode"   tests/dos/dpmi.out $DM $mode -m 386 -L 20000000 tests/dos/dpmi.com
    check "dpmi32 $mode" tests/dos/dpmi.out $DM $mode -m 386 -L 20000000 tests/dos/dpmi32.com
done

if [ -f disks/tp55/TPC.EXE ]; then
    rm -f disks/tp55/HELLO.EXE
    check "tpc -V" tests/dos/tpc.out $DM -V disks/tp55/TPC.EXE hello.pas
    check "tp hello -V" tests/dos/tphello.out $DM -V disks/tp55/HELLO.EXE
    for mode in -j -V; do
        got=$(printf '\033flhello.pas\r\033rr\033x\033x' | $DM $mode disks/tp55/TURBO.EXE 2>&1)
...
if [ -f disks/cobol50/COBOL.EXE ]; then
    ...
    got=$($DM -V -C tmp/cobtest -L 4000000000 tmp/cobtest/COBOL.EXE "DIOPHANT;" </dev/null 2>/dev/null | tr -d '\r')
    case "$got" in *"no errors"*) echo "ok   cobol compile -V";; *) echo "FAIL cobol compile -V"; echo "$got" | tail -3; fail=1;; esac
    $DM -V -C tmp/cobtest -L 4000000000 tmp/cobtest/LINK.EXE "diophant,,,lcobol+cobapi/nod/st:8192;" </dev/null >/dev/null 2>&1
    # the linker names its output as the command line spelled it (diophant.exe): a case-blind host finds it either way
    exe=$(ls tmp/cobtest | grep -i '^diophant\.exe$' | head -1)
    if [ -n "$exe" ]; then echo "ok   cobol link -V"; else echo "FAIL cobol link -V"; fail=1; fi
    got=$(printf '3\r5\r1\r' | $DM -V -C tmp/cobtest -L 4000000000 "tmp/cobtest/$exe" 2>/dev/null | tr -d '\r')
...
if [ -f disks/WP51/WP.EXE ]; then
    ...
    got=$( (sleep 2; printf 'The quick brown fox.'; sleep 2) | $DM -L 20000000000 -C disks -D - disks/WP51/WP.EXE 2>/dev/null)
...
if [ -f disks/djgpp/bin/djecho.exe ]; then
    ...
    for mode in -i -j -V; do
        got=$($DM $mode -m 386 -L 20000000000 disks/djgpp/bin/djecho.exe hello from djgpp 2>&1)
        [ "$got" = "hello from djgpp" ] && echo "ok   djecho $mode" || { echo "FAIL djecho $mode"; echo "$got" | head -5; fail=1; }
    done
    got=$($DM -i -m 386 -L 20000000000 -C disks/djgpp/bin disks/djgpp/bin/stubedit.exe -v djecho.exe 2>&1)
...
if [ -f disks/doom/inst/DOOMS/DOOM.EXE ]; then
    ...
    for mode in -i -j -V; do
        ...
        got=$($DM $mode -m 386 -W -C $D -L 150000000 -G disks/doom/check.png $D/DOOM.EXE </dev/null 2>&1)
        case "$got" in
