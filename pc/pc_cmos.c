 ...
 *
 * The clock registers read the host's local time in BCD, 24-hour, on
 * every access; writes to them are accepted and forgotten (DOS's TIME
 * and DATE set DOS's clock, which is all a guest needs). The rest of the
 * 128 bytes is RAM, with the configuration a POST would leave: diskette
 * types, equipment, base and extended memory, and the checksum over
 * 10h-2Dh.
 */
#include "pc.h"
#include <string.h>
...
static uint8_t cmos_read(uint8_t idx) {
    idx &= 0x7F;
    if (idx <= 0x09 || idx == 0x32) {
        time_t t = time(NULL);
        struct tm tm;
        localtime_r(&t, &tm);
        switch (idx) {
        case 0x00: return bcd(tm.tm_sec > 59 ? 59 : tm.tm_sec);
        ...
        case 0x32: return bcd(19 + tm.tm_year / 100);
        default:   return nvram[idx];            /* alarm registers */
        }
    }
    if (idx == 0x0C) { uint8_t v = nvram[0x0C]; nvram[0x0C] = 0; return v; }   /* flags clear on read */
    return nvram[idx];
}
...
int pc_cmos_port_write(uint16_t port, uint32_t val) {
    ...
    if (port == 0x71) {
        uint8_t i = index_reg;
        if (i > 0x09 && i != 0x0A && i != 0x0C && i != 0x0D && i != 0x32) nvram[i] = (uint8_t)val;
        return 1;
    }
    return 0;
}
